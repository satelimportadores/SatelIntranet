...
include('class.conexion.php');
	...
	if (isset($_REQUEST['ref'])){
		...
		$query = "SELECT * from intranet_dotaciones_inventario where referencia = '$ref'";
		$Rinventario = $inventario->query($query) or trigger_error($inventario->error);
		$inventario->close();
		while ($row = $Rinventario->fetch_array()) {
		echo "<input type='radio' name='tipo' value='".$row['id']."' required>".'<b> Tipo: </b>'.$row['descripcion'].'<b> Talla: </b>'.$row['talla'].'<b> Estado: </b>'.$row['estado'].'<b> Cantidad: </b>'.$row['cantidad'].'<br>';
		};
