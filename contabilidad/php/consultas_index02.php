...
<?php 
  ...
  $alisto = new Conexion;
 $sql01 = "select `irp`.`encargado_alistamiento` AS `nombre`,count(`irp`.`encargado_alistamiento`) AS `cant`,(select count(`intranet_registro_pedido`.`encargado_alistamiento`) from `intranet_registro_pedido` where ((`intranet_registro_pedido`.`encargado_alistamiento` is not null) and (date_format(`intranet_registro_pedido`.`fecha`,'%Y-%m') = date_format(now(),'%Y-%m')))) AS `total`,(select `iu`.`color` from `intranet_usuarios` `iu` where (`irp`.`encargado_alistamiento` = convert(`iu`.`nombre` using utf8)) limit 1) AS `hexcolor` from `intranet_registro_pedido` `irp` where ((`irp`.`encargado_alistamiento` is not null) and (date_format(`irp`.`fecha`,'%Y-%m') = date_format(now(),'%Y-%m'))) group by `irp`.`encargado_alistamiento` order by count(`irp`.`encargado_alistamiento`) desc";
...
//traer encargados de alistamiento:
 ...
 $reviso = new Conexion;
 $sql01 = "select `irp`.`encargado_revision` AS `nombre`,count(`irp`.`encargado_revision`) AS `cant`,(select count(`intranet_registro_pedido`.`encargado_revision`) from `intranet_registro_pedido` where ((`intranet_registro_pedido`.`encargado_revision` is not null) and (date_format(`intranet_registro_pedido`.`fecha`,'%Y-%m') = date_format(now(),'%Y-%m')))) AS `total`,(select `iu`.`color` from `intranet_usuarios` `iu` where (`irp`.`encargado_revision` = convert(`iu`.`nombre` using utf8)) limit 1) AS `hexcolor` from `intranet_registro_pedido` `irp` where ((`irp`.`encargado_revision` is not null) and (date_format(`irp`.`fecha`,'%Y-%m') = date_format(now(),'%Y-%m'))) group by `irp`.`encargado_revision` order by count(`irp`.`encargado_revision`) desc";
...
//traer datos para canvas de grafica
      ...
      $sql01 = "SELECT YEAR(fecha) as year, MONTH(fecha) as mes, MONTH(fecha) as mes01, COUNT(id) as cant FROM intranet_registro_pedido WHERE DATE_FORMAT(fecha,'%Y') = DATE_FORMAT(NOW(),'%Y') AND  recibido = 1 GROUP BY mes";
...
//traer encargados de alistamiento GRAP:
 ...
 $sql01 = "select `irp`.`encargado_alistamiento` AS `nombre`,count(`irp`.`encargado_alistamiento`) AS `cant` from `intranet_registro_pedido` `irp` where ((`irp`.`encargado_alistamiento` is not null) and (date_format(`irp`.`fecha`,'%Y-%m') = date_format(now(),'%Y-%m'))) group by `irp`.`encargado_alistamiento` order by count(`irp`.`encargado_alistamiento`) desc";
